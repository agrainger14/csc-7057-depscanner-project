        ...
        <Box py={3}>
          ...
          <Box py={0.5}>
            ...
            <Grid container spacing={2} alignItems="center">
              <Grid item>
                <Typography sx={{ fontWeight: 500, pb: 0.5 }}>
                  Vulnerability ID:
                </Typography>
              </Grid>
              <Grid item>
            ...
            <Grid container alignItems="center">
              <Grid item>
                <Typography sx={{ fontWeight: 500, pb: 0.5, pr: 2 }}>
                  Affected Dependency:
                </Typography>
              </Grid>
              <Grid item>
            ...
            {affected.map((item, index) => (
              <Box key={index}>
                <Box sx={{ display: 'flex', alignItems: 'center', mb:1}}>
                  ...
                  {item.ranges[0].events[1].last_affected && 
                    <Typography sx={{ color: 'error.main' }}>
                ...
                {item.versions && 
                  <Button variant="outlined" sx={{mb:1}} onClick={() => handleModalOpen(index)}>
                ...
                <Dialog open={isModalOpen} onClose={handleModalClose} sx={{overflow: 'hidden' }}>
                  ...
                  <DialogContent>
                    {selectedIndex !== null && affected[selectedIndex].versions && (
                      affected[selectedIndex].versions.map((version, index) => (
                        <RouterLink
                          key={index}
                          style={{ color: 'inherit' }}
                          to={`/dependency/${encodeURIComponent(affected[selectedIndex].package.name)}/${affected[selectedIndex].package.ecosystem.toUpperCase()}/${version}`}
                        >
